...
def custom_collate_fn(batch):
    ...
    u1_batch, u2_batch = [], []
    u1_lens, u2_lens = np.zeros(bt_siz, dtype=int), np.zeros(bt_siz, dtype=int)
    ...
    t1, t2 = u1_batch, u2_batch
    u1_batch = Variable(torch.ones(valid_ind, l_u1).long() * pad_idx, requires_grad=False)
    u2_batch = Variable(torch.ones(valid_ind, l_u2).long() * pad_idx, requires_grad=False)
    u1_lens, u2_lens = u1_lens[:valid_ind], u2_lens[:valid_ind]
    
    end_tok = torch.LongTensor([2])

    for i in range(valid_ind):
    ...
    sort1, sort2 = np.argsort(u1_lens*-1), np.argsort(u2_lens*-1)
    # cant call use_cuda here because this function block is used in threading calls

    return u1_batch[sort1, :], u1_lens[sort1], u2_batch[sort2, :], u2_lens[sort2]
...
class MovieTriples(Dataset):
    def __init__(self, data_type, length=None):
        ...
        elif data_type == 'subtle':
            _file = '/home/harshals/hed-dlg/Data/MovieTriples/Subtle_Dataset.triples.pkl'
        self.utterance_data = []
